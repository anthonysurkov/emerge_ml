...
NUM_SEQS = 1000
NUM_EPOCHS = 10 # R255X seems to plateau around ~40 epochs
NUM_SUBOPTS = 256

PT_DATA_INFILE = f"data/training/r255x_subopts_{NUM_SEQS}_k{NUM_SUBOPTS}_lik.pt"
MODEL_OUTFILE = f"data/models/r255x_{NUM_SEQS}_k{NUM_SUBOPTS}_e{NUM_EPOCHS}_lik.pt"
PERFORMANCE_DATA_OUTFILE = f"data/performance/r255x_{NUM_SEQS}_k{NUM_SUBOPTS}_e{NUM_EPOCHS}_lik.npz"
...
def binom_nll(logit_p, k, n):
    """Mean negative binomial log-likelihood over the batch."""
    return F.binary_cross_entropy_with_logits(
        logit_p,
        k / n.clamp_min(1.0),
        weight=n / n.mean(),    # normalize weights around 1.0
        reduction="mean"
    )
...
class ViennaLandscapeDataset(Dataset):
    ...
    def get(self, i: int):
        ...
        if self.n is not None:
            out["n"] = self.n[i].float()
            out["k"] = self.k[i].float()
        return out

def collate_sequence_of_graph_sets(batch):
    all_graphs, counts, idxs = [], [], []
    ns, ks = [], []
    for item in batch:
        gs = item["data_list"]
    ...
    if ns:
        out["n"] = torch.stack(ns)
        out["k"] = torch.stack(ks)
    return out
...
def train_one_epoch(model, loader, optim, device, epoch=0):
    ...
    for batch in pbar:
        ...
        loss = binom_nll(pred, k, n)

        optim.zero_grad()
        loss.backward()
        optim.step()

        total += loss.item() * n.numel()
        n_samples += n.numel()
        pbar.set_postfix(loss=total / max(n_samples, 1))
...
def eval_one_epoch(model, loader, device, desc="eval", log_every=20):
    ...
    for i, batch in enumerate(pbar, 1):
        ...
        total += binom_nll(pred, k, n).detach()   # tensor on device
        n_samples += n.numel()

        if (i % log_every) == 0 or i == 1:
            pbar.set_postfix(mse=(total / max(n_samples, 1)).item())

    return (total / max(n_samples, 1)).item()
...
def predict_all(model, loader, device):
    ...
    for batch in loader:
        ...
        ns.append(batch["n"].cpu())
        ks.append(batch["k"].cpu())
    return (torch.cat(idxs).numpy(), torch.cat(preds).numpy(),
            torch.cat(ns).numpy(), torch.cat(ks).numpy())
...
def main():
    ...
    idx, pred_p, n, k = predict_all(model, val_loader, device)
    order = np.argsort(idx)
    idx, pred_p, n, k = idx[order], pred_p[order], n[order], k[order]
    p_empirical = k / np.maximum(n, 1)
    ...
    ss_res = ((p_empirical - pred_p) ** 2).sum()
    ss_tot = ((p_empirical - p_empirical.mean()) ** 2).sum()
    r2 = 1.0 - ss_res / ss_tot
    print("R² (prob scale):", r2)
    
    plt.figure()
    plt.plot(idx, p_empirical, label="k/n (empirical)")
    plt.plot(idx, pred_p, label="pred p")
    plt.xlabel("idx")
    plt.ylabel("p")
    plt.legend()
    plt.tight_layout()
    ...
    plt.figure()
    plt.scatter(p_empirical, pred_p, s=12)
    plt.xlabel("k/n (empirical)")
    plt.ylabel("pred p")
    plt.tight_layout()
    plt.show()
    
    np.savez(PERFORMANCE_DATA_OUTFILE, idx=idx, pred=pred_p, n=n, k=k)
